...
router.get('/requests/*', async (req, res, next) => {
  ...
  try {
    ...
    if (storedAssertion) {
      ...
      setEmitter('requests', MyEventEmitter.getEmitter('assertionRequest'), eventPath, res)
    }
  } catch (err) {
    next(err)
  }
...
const setEmitter = async (object, emitter, eventPath, res) => {
  ...
  emitter.once(eventPath, async (data) => {
    clearTimeout(timer)
    await assertionStore.pop(object, eventPath)
...
router.get('/callbacks/*', async (req, res, next) => {
  ...
  try {
    ...
    if (storedAssertion) {
      ...
      setEmitter('callbacks', MyEventEmitter.getEmitter('assertionCallback'), eventPath, res)
    }
  } catch (err) {
    next(err)
  }
...
// const helper = (res, eventPath, emitter, object) => {
//   const timer = setTimeout(() => {
//     emitter.removeAllListeners(eventPath)
//     res.status(500).json({ error: 'Timed out' })
//   }, LONGPOLLING_TIMEOUT)
