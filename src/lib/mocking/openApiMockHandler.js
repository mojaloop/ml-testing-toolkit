...
const generateAsyncCallback = async (item, context, req) => {
  ...
  if (generatedCallback.body) {
    ...
    switch (context.request.path) {
      ...
      case '/transfers': {
        const transferCallback = Object.assign({}, generatedCallback)

        transferCallback.headers['FSPIOP-Destination'] = context.request.body.payerFsp
        await CallbackHandler.handleCallback(transferCallback, context, req)
