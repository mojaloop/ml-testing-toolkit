...
const processTestCase = async (testCase, traceID, inputValues, variableData, dfspId, globalConfig) => {
  ...
  const apiDefinitions = workerData.apiDefinitions
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      ...
      if (successCallbackUrl && errorCallbackUrl && (ignoreCallbacks !== true)) {
        timer = setTimeout(() => {
          workerData.testOutboundEventPort.removeAllListeners()
        ...
        workerData.testOutboundEventPort.postMessage( { action: 'listenCallbacks', successCallbackUrl, errorCallbackUrl} )
        // Listen for callbacks
        workerData.testOutboundEventPort.once('message', (data) => {
          clearTimeout(timer)
          const { callbackHeaders, callbackBody } = data
          if (data.callbackType === 'successCallback') {
          ...
          } else if (data.callbackType === 'errorCallback') {
...
if (workerData.action === 'OutboundSend') {
  OutboundSend(workerData.inputTemplate, workerData.traceID, workerData.dfspId)
}
