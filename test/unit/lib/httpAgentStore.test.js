...
describe('httpAgentStore', () => {
  ...
  describe('stop', () => {
    it('Stop should clear interval and destroy all agents', async () => {
      const httpAgent = httpAgentStore.getHttpAgent('test-http', {})
      const httpsAgent = httpAgentStore.getHttpsAgent('test-https', {})
      expect(httpAgent).not.toBeNull()
      expect(httpsAgent).not.toBeNull()

      const result = httpAgentStore.stop()
      expect(result).toBeUndefined()
    })
    it('Calling stop multiple times should not throw an error', async () => {
      httpAgentStore.stop()
      const result = httpAgentStore.stop()
      expect(result).toBeUndefined()
    })
    it('Can reinitialize after stopping', async () => {
      ...
      SpyGetSystemConfig.mockReturnValue({
        ...
        HTTP_CLIENT: {
          KEEP_ALIVE: true,
          MAX_SOCKETS: 50,
          UNUSED_AGENTS_EXPIRY_MS: 1000,
          UNUSED_AGENTS_CHECK_TIMER_MS: 5000
        }
      })
      httpAgentStore.init()
      const agent = httpAgentStore.getHttpAgent('test-reinit', {})
      expect(agent).not.toBeNull()
      httpAgentStore.stop()
      SpyGetSystemConfig.mockClear()
    })
    it('should handle agent destroy errors gracefully', async () => {
      jest.useFakeTimers()
      SpyGetSystemConfig.mockReturnValue({
        API_DEFINITIONS: [],
        HTTP_CLIENT: {
          ...
          MAX_SOCKETS: 50,
          UNUSED_AGENTS_EXPIRY_MS: 0,
          UNUSED_AGENTS_CHECK_TIMER_MS: 1000
        }
      })
      httpAgentStore.init()
      
      // Create an agent
      const agent = httpAgentStore.getHttpAgent('test-error', {})
      expect(agent).not.toBeNull()
      ...
      // Should not throw error even if destroy fails
